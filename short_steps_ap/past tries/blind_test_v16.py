...
def run_v16_test():
    ...
    model.load_state_dict(torch.load("anc_v16_energy.pth", map_location=device))
    model.eval()

    print("--- v16 Energy-Matched Blind Test (Negative dB = SUCCESS) ---")
    results = []
    with torch.no_grad():
    ...
    plt.semilogy(target['f'], target['p_resid'], color='green', label="v16 Residual")
    plt.fill_between(target['f'], target['p_resid'], target['p_orig'], where=(target['p_resid'] < target['p_orig']), color='green', alpha=0.2)
    plt.title(f"v16 Energy-Matched Result: Scenario {target['id']} ({target['total']:.2f} dB)")
    plt.xlim(0, 2200); plt.grid(True); plt.legend()
    plt.savefig("v16_blind_test_report.png")
