...
class PhaseInvertedTCN_v17(nn.Module):
    # (Architecture identical to trainer)
    def __init__(self):
...
def run_v17_test():
    ...
    model.load_state_dict(torch.load("anc_v17_phase_inverted.pth", map_location=device))
    model.eval()

    print("--- v17 Final Blind Test (Negative dB = SUCCESS) ---")
    results = []
    with torch.no_grad():
    ...
    plt.semilogy(target['f'], target['p_orig'], label="Original Noise", alpha=0.4)
    plt.semilogy(target['f'], target['p_resid'], color='green', label="v17 Final Residual")
    plt.fill_between(target['f'], target['p_resid'], target['p_orig'], where=(target['p_resid'] < target['p_orig']), color='green', alpha=0.2)
    plt.title(f"v17 Final Result: Scenario {target['id']} ({target['total']:.2f} dB)")
    plt.xlim(0, 2200); plt.grid(True); plt.legend()
    plt.savefig("v17_final_blind_test_report.png")
